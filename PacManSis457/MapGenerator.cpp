	...
	while (getline(file, line)) {
		...
		for (unsigned int x = 0; x < chars.size(); x++) {
			...
			/*case '.':
				...
				break;*/
			case '.':
				objetoNuevo = new MonedaClasica(tileNuevo, textureManager->getTexture("monedaClasica"), x * 25, y * 25, 25, 25, anchoPantalla, altoPantalla);
				objetoNuevo->setParametrosAnimacion(4);
				break;
			case '-':
				objetoNuevo = new MonedaGalactico(tileNuevo, textureManager->getTexture("monedaGalactico"), x * 25, y * 25, 25, 25, anchoPantalla, altoPantalla);
				objetoNuevo->setParametrosAnimacion(4);
				break;
			case 'p':
