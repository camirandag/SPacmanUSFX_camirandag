...
void Pacman::update()
{
	...
	if (tileActual != nullptr && tileActual->getMonedaBase() != nullptr) {
		...
		if (revisarColision(eatingHole, tileSiguiente->getMonedaBase()->getColisionador())) {
			tileSiguiente->getMonedaBase()->deleteGameObject();
		}
	}
